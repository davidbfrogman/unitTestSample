...
export class xQueryBuilder {
    ...
    withConfig(configuration?: xQueryConfigType): xQueryBuilder {
        ...
        //Required Properties
        this.entity = configuration.entity !== undefined ? configuration.entity : null;
    ...
    build() {
        ...
        this.searchOrganizers.forEach((so,soIndex) => {
            ...
            so.attributeOrganizerList.forEach((ao,aoIndex) => {
                if (ao && ao.attribute && ao.operationType) {
                    ...
                    if ((ao.attribute.AttributeType == AttributeType.DateTime) ||
                        ao.attribute.AttributeType == AttributeType.Timestamp) {
    ...
    public getDateFormattedForSearch(dateString: string): string {
        ...
        let dt = moment(dateString, Constants.SOHO_DATEPICKER_DATE_FORMAT);
        return dt.format(Constants.REST_API_SEARCH_DATE_FORMAT); //Moment uses [] for escape chars.
    }

    public getTimeFormattedForSearch(timeString: string): string {
        //15:03:00 -> 15:03:00Z
        let dt = moment(timeString, Constants.SOHO_TIMEPICKER_TIME_FORMAT);
        return dt.format(Constants.REST_API_SEARCH_TIME_FORMAT);
    }
}
